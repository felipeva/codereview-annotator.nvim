...
| `codereview/leading_type_child.lua` | Spawned six times by `panel_spec`, one painted cell each — a file's **state** mark in its **leading type**'s colour, on a row with no line-wide group, on the row the diff cursor is in, on that row again with a foreground added to `CursorLine`, which is the control that says which row loses the colour, and on the first row again after its bug was made a nitpick with `t` in the queue float, and once more after the same change made with `ct` over the diff — deliberately not a spec. |
...
| `codereview/edit_child.lua` | Spawned by `edit_restart_spec` to queue three bugs from a buffer, edit the middle one's note, take the type off the first and make the last a nitpick, and exit with nothing written after that — deliberately not a spec. |
...
| Spec | Covers |
| --- | --- |
| `types_spec` | Configuring annotation types: defaulting, validation, grouping, a custom type end to end — and the glyphs the five shipped types carry: that each has one, that it collides with nothing else the plugin draws, that it spends one display column, and that it comes out of no patched font's private use area; beside them, the rule that an empty `icon`, `label`, `hl` or `directive` is *absent* rather than a value, which is what an empty string being truthy in Lua had hidden. Also the type picker's rows over a **replaced** vocabulary: that a host's own names, keys and directives are what is offered, that a type with no directive stops at its key, and that the glyph, name and key are columns — which only a host can break, by an empty glyph or a two-cell one |
| `diff_spec` | Scope resolution, unified-diff parsing, rename/binary/untracked, blob hashing |
| `render_spec` | Anchor map, byte columns, navigation, collapse, panel, scope cycling, archived entries on the diff — where they draw, the groups they draw in, the annotation type's glyph in front of a queued entry and the same glyph in front of an archived one, in the archive's own group, what they cost a file they say nothing about, the flag that removes them and the key that overrides that flag in both directions, for a session — how a winbar is assembled from typed segments, with no review, no fixture and no repository behind it — including the **group** a segment carries, which is a name the plugin did not choose once a `file_icon` adapter names it: a name that would break out of its `%#...#` marker refused, the expression it would otherwise have run proved to run when the marker is written by hand, exactly the character set Neovim itself accepts, and the three states a group can be in as the parser reports them — defined, emptied, never defined — which is what says a bar reading cannot tell an undefined group from no group at all; then the unified layout's sticky header: the file under the cursor, the crossing, the rename spelled out, which group every piece of the bar draws in, what a narrow pane sheds and what the glyphs bought the path, the tree dismissed, a name carrying a `%`, and a review with no files |
| `split_spec` | The split layout: pane parity, anchor totality, filler, per-pane chrome and note mirroring — queued and archived alike — with no windows; a note in a narrow pane wrapped rather than clipped, and every row it continues onto indented to the column the prose above it starts at, in both layouts, which is the one place the marker's width in *columns* is read back; then the binding, annotation parity against the unified layout, the two intersections nobody else owns, each pane's winbar naming its own side of a rename, and the painted cells proving that bar mutes with its pane and draws its counts in a group of the plugin's own |
...
| `frame_spec` | The **frame**: what marks a file out in a review. The four groups first — definitions rather than links, because a link cannot add an attribute to what it copies; a **band** carrying a background and no foreground; that band standing off the theme's `CursorLine` by a *minimum distance per channel* — a floor calibrated by the 10% band that was rejected, which a bare inequality passes at four units — and landing *strictly* between `Normal`'s two colours, because the arithmetic itself is asserted on a painted cell and not repeated here; one band for both file states, with what says a file is reviewed left to the column mark under it; no gui rule anywhere and nothing at all on the **pad** group; the `cterm` underline unchanged on all four, which is the terminal that can compute no band; no `overline`; named where `hl.groups()` reads, with a muted twin and a faded twin that keep the band, and no twin at all for a pad group with no colour left to blend. Then which row carries which line-wide group, at the pure-data seam: an expanded file banded on its own header anchor and closed on its own pad anchor with nothing framed between them, its header group still under the band as a column mark, a collapsed file and a reviewed one banded with no pad group, a reviewed file expanded again drawing both rows from the reviewed pair, the last file, a review of one file, a review of none, a **solo** render banding the file it draws, a file with two hunks whose *last* pad row alone closes it, a file whose whole body is a note, both **panes** of a split banding the same rows in the same groups — including a before pane's *empty* header row — and the mark count that moves by one when a file collapses. Then a `:colorscheme` onto a *light* theme, last in the file and putting the theme back: the band recomputed against the theme that is active now, landing strictly inside the new `Normal` — which a backdrop memoised across the change cannot, since it lands the band on that theme's foreground exactly — clearing the light cursor line by the same floor, and its twins written again. Then four painted cells in child processes, which is the only claim a group name cannot make: the band's own colour computed from that process's `Normal`, a column mark keeping its foreground over it, the two read as an inequality of foregrounds and an equality of backgrounds because each absolute reading passes for one reason and fails for two, the band muted with its pane, and no rule left on a pad row the render really marked. |
...
| `solo_spec` | Drawing one file and none of the others, in two acts. With no windows: the configuration option's default, then the render told which file to draw — the one file that has rows, its header row at its own true index with no other index in the map, every anchor naming it and no other, the file drawn exactly as the full render draws it, the render that is unchanged when it is told nothing, the soloed split's two panes of one file's height with one header row between them, the collapsed file that keeps its header, and the empty scope that draws an empty review. Then in a view: the review that opens on one file, the tree still listing every file with its marks and its counts, the summary still counting the whole scope, `]f` and `[f` — including `[f` from inside a file going to the top of it, which is the half of the old row rule an index rewrite can silently drop — `]F` and `[F` over a map with one entry that is not at index 1, which is the sparse-map bug and needs more than one file left unreviewed, `<C-p>` and the tree's `<CR>` each reaching a file that is not drawn, the queue float listing every entry and its jump drawing another file, an archived entry on the file being read, the entry an annotation produces being the one it produces unsoloed, `]h` and `[h` stopping at the drawn file's last and first hunk -- which they do by construction rather than through a branch -- and crossing files again with solo off, `]a` and `[a` coming back to the drawn file's own note while a second note sits in a file that is not drawn, `R` marking and going on to the next unreviewed file from the diff and from the tree, wrapping in the motion's own words and saying the review is done on the last one with the view left on it, the unmark and the directory row that go nowhere, `R` collapsing and staying put with solo off, and the option turned off in a live review |
| `solo_toggle_spec` | The key on the switch in front of solo: `go` bound in the diff and in the tree, one file drawn from the file the cursor was in and every file drawn again with that same file still under it, the configured value left where the host set it, the choice kept across a repaint, and `gl` — a soloed split drawing one file in both panes — `gr`, `gA`, `gp`, `za`, `gS` and `gs` round the whole scope cycle each leaving it alone; then what the session does not outlive, in two child processes configured opposite ways: the note each leaves in the store both sessions share, the switch neither writes into it, and the configured value this one starts from in both directions |
| `path_spec` | How a file's path is styled: its directories quiet, its own name bright. Four acts. The segments the label answers with, with no review, no fixture and no repository — a plain path's two halves, the cut at the *last* separator, a root-level file with no quiet half, a path that is not ASCII, a rename's two paths and its chrome arrow in the unified layout and one path per pane in the split one, the pre-image half a file added on the branch does not have, a name carrying a `%` staying a literal, and the fields every other surface goes on reading off that label. Then the **byte offsets** the header row colors at, off `render.build`, which is data in and data out — the guard that `"○ ▾ "` is eight bytes and four columns, a path built by hand with an accent in it because no fixture here has a non-ASCII path, the root-level file, the rename, a header cut to fit with every mark still inside its row, the stat and the note count that did not move, solo, and the before pane naming its own side. Then the two groups — in the set the muting derives its namespace from, linked into the colorscheme, each with a fade twin holding a color, and resolving to two colors so the rule is visible at all. Then the cut a narrow pane makes, over segments rather than a string. Last, in a real view, the one claim no single surface can make: the label's answer is replaced and **both** the header row and the sticky header say what it says |
| `file_icon_spec` | The `file_icon` adapter: a host's own glyph for a file and the group that colours it, beside the mark that says what the reviewer has done with it. Four acts. What the adapter is handed and what its answer becomes, with no review and no fixture — the file's own path and a rename's post-image one, asked exactly once; the glyph arriving beside the state mark rather than in place of it, on a reviewed file and on an annotated one; the prefix both surfaces draw, spelled once; and the row a file with no adapter draws, asserted as **eight bytes** rather than as a shape. Then the **group** the rule carries out beside the glyph — following the file as the glyph does, absent for an adapter that answered with a glyph alone and for one that raised, and dropped on its own when it is a number, a table, an empty string or a boolean, with the glyph kept: a broken group reaches an extmark as a name and would raise on the paint that emits it — and dropped too when it is a **string that could not be a highlight group**, `A#B` and a name with a space among them, because the sticky header is a statusline and a `#` in a group name ends its marker early, with the names a host really answers with kept beside them so the rule is not "drop every group there is". Then the five ways an adapter can fail to answer — raising, nil, an empty string, a number, a table, and a host that wired something that is not a function — every one of them ending in the row nothing wired draws; and the key the plugin ships nothing behind, which is what makes "nothing is called per file" a structural rule rather than a habit. Then the header row off `render.build` — the glyph's **bytes** moving the path's two ranges, the state mark still first on the row, one adapter call per file drawn and exactly one under **solo**, a row byte-identical to the unwired one when the adapter raises on every file, the stat still against the right margin under a glyph two columns wide, and the before pane's indent measured past the glyph — then the **colour** on that row: the adapter's group over the glyph's own bytes read back off the row, the range placed by **bytes and not columns** — eight bytes and four columns of head — two files carrying two colours, no range at all for an adapter that answered with a glyph alone, the path's own two ranges still at the offsets the act above asserts them at, the range kept inside a row a narrow pane cut, and the before **pane**, which draws no glyph and therefore carries no colour for one; then **the truncation trap, pinned as an equality**: an adapter answering with a glyph *and* a group draws the rows, the before pane, the label and every mark a glyph-alone adapter draws **apart from the glyph's own range** — that one exception named and counted rather than filtered away silently — in both layouts at three widths, with each of those ranges **read back off its row** rather than counted, because a count sits still while a range moves under the narrowest width — because `file_label` captures the group beside the glyph now and an edit that put it anywhere near the prefix would move every byte offset on every header row. Then the **file tree** off `panel.build`, which is data in and data out as the render is, on file lists built by hand rather than on `panel_spec`'s nested fixture — whose row assertions are spelled glyph by glyph and would all move if an adapter were wired into its process: the glyph between the state mark and the name, the mark still first in its own column on all three states with the *leftmost coloured range* read back off the row, the whole path handed to an adapter that a basename is drawn from, every file asked about once and no directory asked about at all, the directory row and its compacted chain byte-identical either way, the five broken answers ending in the unwired row, a panel too narrow keeping the mark and the glyph and cutting the name, a name that is not ASCII cut on a character boundary, an empty **scope**, and the rule reached once per file when an adapter is wired and **not at all** with nothing wired — which is the performance rule and the one claim a row cannot make, because a `pcall` over a nil adapter draws the same row a guard does. The **colour** on that surface sits in the same act: the adapter's group over the glyph's own bytes read back off the row, two files carrying two colours, the range placed by **bytes and not columns** — six bytes and four columns of head — the state mark still leftmost in its own group on all three states, a glyph with no colour carrying the state mark's range and no other, each broken group leaving every line and every mark equal to a glyph-alone adapter's, a panel too narrow keeping the colour with its range inside the row, and the **fade**'s twin computed for a group this plugin has never heard of, with the group and the twin put back before the assertions run. Then the **directory row**, in the same act and on the same hand-built lists, through `dir_icon` — a second adapter with `file_icon`'s contract, whose answer the same rule reads: the glyph after the chevron, a compacted chain drawing the glyph of the directory its row *names* and not of the head of the chain it swallowed, a collapsed directory drawing its glyph as an expanded one does, the counts still against the right margin, each adapter asked once per drawn row and never handed the other's kind of thing, a directory the tree did not draw asked about not at all, the host's group over the glyph's bytes — placed by bytes and not by columns, on a head of four bytes and two columns — with the row's own group laid *around* it rather than under it, so which colour draws does not rest on which extmark was emitted last, the five broken answers and the five broken groups each ending in the row a glyph-alone adapter draws — the fifth of those being a name that could not be a highlight group, which reaches a directory row through the rule both adapters share rather than through anything on this surface, a narrow panel cutting the name and never the chevron or the glyph, the rule reached once per drawn directory row and **not at all** with nothing wired, and the unwired row pinned mark for mark as well as byte for byte. Last, in a real view opened three times — first with nothing wired, because `setup` is a process — the same glyph on the header row and on the **sticky header**, the tree row unchanged with nothing wired, the glyph a tree row draws read back and compared against the one that file's header row draws, a `%` in one drawn literally on the bar, a file marked reviewed keeping its mark on both surfaces with the glyph beside it, and then the same review reopened with a **group-answering** adapter: the header row and the whole sticky header byte-identical to what the glyph-alone adapter drew and its marks identical **but for the glyph's own range**, the head carrying two ranges where it carried one — the row's own quiet, and the glyph's colour — the bar's glyph in the group the adapter named with the separator behind it left in the bar's own — the extent, not only the group — read with that group **defined** and put back to the state it was found in, the tree row carrying the injected group, which is the guard that says the injection reached anything at all, and last the glyph *and the group* a tree row draws compared against the ones that file's header row draws, on two files, each side found by the group's **name** rather than at the offset the case expects |
| `chrome_spec` | Three details of the review view's chrome, each at the narrowest seam that can see it. The **unified** hunk header off `render.build`, on a diff git wrote and `diff.parse` read back — git's own header string and nothing appended, the heading counted so the case says which half is wrong, a hunk git appended nothing to, and the **split** layout's two headers, which no fixture in this suite could pin. Then what a base revision is *called*, at `render.rev_label`, with no review, no fixture and no repository — the object name abbreviated to seven, `:0` reading `index`, a branch, a tag and two short names left whole, a 40-character name that is not hexadecimal left whole, and `vim.system` stubbed to prove no process is spawned to decide. Last the review buffer's name, live: the **scope** rather than the buffer number, the file tree's and a before **pane**'s buffers keeping the numbered form, `gs` renaming through the key, and a second review of one scope opening under `codereview://<scope>#<bufnr>` — with the E95 collision itself asserted while that review is open, because a name that merely differs is also what numbering every second review would produce, and `name_review`'s own answer read back, because a buffer that kept a previous scope's name reads exactly like one that was named for it. Then a **revspec**, which is the one scope whose name is a category and which is therefore named for its label |
...
| `annotate_spec` | Targeting, cross-file clamp, deleted-line rule, types, drop, grouping, and editing a queued note over a real review: the note replaced and nothing else, id and place kept, a stale entry still stale, the repaint, the disk, the message, and an abandoned or empty edit changing nothing; then `e` over the diff, through the key: the composer at once with the note on a line with one entry, the picker choosing on a line with more and only that entry edited, a dismissed picker or an abandoned composer changing nothing, the message `x` gives on a line with none, and the diff repainted with the new note; then `ct` over the diff, through the key: the type picker at once on a line with one entry, the entry picker choosing on a line with more and only that entry retyped, either picker dismissed changing nothing, the message `x` gives on a line with none, and the diff repainted in the new type's group |
...
| `capture_spec` | Annotating from an ordinary buffer: scope, types, declining one — with the type picker offering every type behind its own glyph and declining behind the untyped mark, read as each label's first non-blank run because `find` alone cannot fail on an empty glyph; beside it, that each row also names the type's key and its directive, read by the order the three appear in rather than at an offset, and that declining carries a mark and a name and nothing after them — blob, composer, diagnostics, restart, one queue, the immediate send |
...
| `glyph_mute_spec` | A host's icon groups reaching the namespace a **muted** pane draws through: the set the render records, empty with nothing wired and guarded by a case that the review really painted; the group readable from the render and present in the namespace; the twin holding that group's colour pulled toward the background; and the two painted cells that discriminate the focused pane from the muted one. Its own file because the set is module-level and only grows, so "empty with nothing wired" needs a process that has wired none — one per spec file. |
...
| `panel_spec` | Tree build, chain compaction, folding, subtree review, navigation, picker, dismissing and summoning the tree; then the footer's **progress bar** off `panel.build` on file lists built by hand, because a review of exactly twelve files and one of exactly one are both needed and every expected cell count is spelled as a literal — the row one row high with the blank row above it untouched, the bar after the tally and one separating space with nothing else on the row, nought, part-way at two points, one file short of the end and complete, a review of one file and a review of none, the bar's **length** held still across both digit steps at a hundred files, the clamp that gives a started review its first cell, the glyphs a host replaced, a panel too narrow for a bar leaving the tally byte for byte what it was, the row's marks pinned as the one line-wide group it has always carried — which is what says the bar is not drawn in a finished directory's green — and a real review repainting the bar when a file is marked reviewed; and last the three painted cells, which are the only reading that can say what colour the bar came out |
| `queue_float_spec` | How the float draws an entry: the bar down every row it owns, the boundary between two, notes kept and wrapped by display width, dropping, editing and changing the type from anywhere inside one — the current type marked in the picker, typed to untyped and back, and a dismissed picker changing nothing — where the cursor lands after an edit and after a change of type moves the entry to another group, an entry from the capture path and a bare note edited alike, the keys `?` lists and the footer that points there, the title and the footer fitting the float at its narrowest, the mark it spends on a stale entry, learned off the row and refused to every annotation type, and the two keys that act on the whole batch — one closing the float, one leaving it open |
...
| `focus_spec` | Queue-float focus across the async picker, the target the float's title names, submit closing the float, and where a submit under a **preamble** leaves the cursor |
...
- **`mkfixture.sh`** — flat `src/`-only repo covering every file status at once:
  ...
  `faded_spec`, `quiet_spec`, `frame_spec`, `edit_restart_spec` and `interactive_spec`. Its
...
- **A probe that widens a marker has to widen it with a multibyte glyph.** Widening the
  marker in front of a note and running the suite is how "nothing on that row depends on its
  width" stops being a reading and becomes a measurement — it finds `quiet_spec`, which is
  the one consumer that reads a note row by *column*, and finds nothing else. Widen it with
  spaces, though, and bytes and columns move together, so the probe cannot see a consumer
  that measures one and pays the other: that is exactly the bug that shipped the first
  glyph, and the suite was green through it. `split_spec` now reads the continuation indent
  back in columns, which is the case that would have caught it.
...
- **A muted winbar cannot be told from a bright one by its text**, and a non-current window
  ...
  only assertion that can see it is the painted cell. `split_spec` spawns `winbar_child.lua`
  three times over one cell inside the path the bar names — focused, muted, and **muted
  off** — and it is the third that gives the second its teeth: without it the muted reading
  is only known to differ from `WinBar`, not to be a blend of anything. **That one cell
  answers about two groups**, because a `%#Group#` names a foreground and leaves the bar's
  background showing through: the foreground is `CodeReviewFileDir`, which the path's quiet
  half carries, and the background is `WinBar`/`WinBarNC`. Both halves are mutation-checked —
  take `WinBarNC` out of `EDITOR_GROUPS` and the background reds, take `CodeReviewFileDir`
  out of `LINKS` and the foreground does — because a cell reading that can only fail on one
  of the two mechanisms it claims to cover is half a decoration. A fourth run reads a different
...
- **A bar reading cannot see a group the theme has never defined, and reports the bar's own
  instead.** `nvim_eval_statusline` resolves each `%#Group#` against the theme and drops a
  marker naming a group nothing defined, so the run underneath it comes back holding only
  `WinBar`/`WinBarNC` — which `h.winbar_group` reports as an absence, exactly as it reports a
  segment that asked for no group at all. The two are indistinguishable from outside. So an
  assertion that a bar draws something in a *host's* group — `MiniIconsAzure`, `DevIconLua`,
  anything this plugin does not link itself — passes for a reason unrelated to its claim
  unless the group is defined first. A group this plugin owns needs none of it: `hl.lua` links
  every one at `setup`, so they are defined in any process that opened a review.
- **There are three states, not two, and the third is what a restore leaves behind.**
  Defined-with-a-colour and defined-but-*emptied* are both reported; only never-defined is
  dropped. There is no API to undefine a group, so a case that borrows one and "restores" it
  with `nvim_set_hl(0, name, {})` leaves it in the middle state — reported — for every case
  after it, because plenary runs one file's `it` bodies top to bottom in one process. Two
  cases in `file_icon_spec` borrow `MiniIconsAzure`: the fade act writes it first and empties
  it, and the bar act after it therefore finds it *already defined*, gives it a colour, and
  writes the empty table back — which is an exact restore of the state it found and not of an
  absence. Do not write a later case as though that group were undefined. `render_spec`'s
  `reports a group the theme defines, and drops one it does not` pins all three states against
  the parser, so the claim in this bullet is a case rather than a note.
...
- **A width budget written against one revision stops guarding when the revision gets
  shorter.** `split_spec`'s sticky-header block widens the terminal so both bars have room for
  everything they name, and guarded that with arithmetic on the before pane: its 40-character
  revision plus the pre-image path against the pane's width. At seven characters that needs
  about 37 columns of 66 and cannot fail — and it could not fail at 40 columns either, so it
  had stopped guarding the widening it was written for. It compares the pane's width against
  what it was *before* the widening now, which can fail whatever either bar happens to need,
  and it is taken on the **after** pane, which is where the room is still tight: that bar
  carries the file and the whole review summary, and a bar with less sheds the review's label
  off its head.
...
- **`truncate` can delete the evidence a row assertion is looking for.** A hunk header that
  ...
  heading appears on that row reads **one** on a row that says it twice — the case passing
  over the exact bug it was written for. `chrome_spec` builds at 120 for that reason and its
  comment says so. The trap is sharper than it looks, because the fault is *worst* on a narrow
  pane: where a reviewer suffers most from it is also where an assertion cannot see it. Same
  trap as "a filter test needs a fixture only that filter can reject".
- **A hunk with a section heading cannot come from any fixture in this suite.** git appends
  the enclosing declaration only when it finds one *above* the hunk, and every fixture file
  here either changes at its first line or has no declaration at all — measured: `mkfixture`
  produces seven hunks and not one heading among them. `split_spec` used to carry a
  `keeps git's section heading with the post-image range` that looped looking for one, found
  none, and passed without asserting anything from the day it was written; it has been
  **deleted**, and `chrome_spec` pins both split headers instead. What `chrome_spec` reads is a
  diff git wrote, put through `diff.parse` rather than typed out as a `CRHunk`: `header` and
  `heading` then agree the way git's own output makes them agree, which a hand-written pair
  cannot be trusted to do, and no count or row assertion in nine other specs has to move.
  Teaching a fixture a function is the alternative, and it moves all of them.
