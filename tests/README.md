...
| `codereview/leading_type_child.lua` | Spawned five times by `panel_spec`, one painted cell each — a file's **state** mark in its **leading type**'s colour, on a row with no line-wide group, on the row the diff cursor is in, on that row again with a foreground added to `CursorLine`, which is the control that says which row loses the colour, and on the first row again after its bug was made a nitpick with `t` in the queue float — deliberately not a spec. |
...
| `annotate_spec` | Targeting, cross-file clamp, deleted-line rule, types, drop, grouping, and editing a queued note over a real review: the note replaced and nothing else, id and place kept, a stale entry still stale, the repaint, the disk, the message, and an abandoned or empty edit changing nothing |
