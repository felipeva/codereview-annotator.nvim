...
| `queue_float_spec` | How the float draws an entry: the bar down every row it owns, the boundary between two, notes kept and wrapped by display width, dropping from anywhere inside one, the mark it spends on a stale entry, learned off the row and refused to every annotation type, and the two keys that act on the whole batch — one closing the float, one leaving it open |
...
- **`mkfixture.sh`** — flat `src/`-only repo covering every file status at once:
  ...
  `faded_spec`, `quiet_spec`, `frame_spec` and `interactive_spec`. Its
