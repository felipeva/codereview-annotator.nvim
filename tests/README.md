...
| `solo_spec` | Drawing one file and none of the others: the configuration option's default, then the render told which file to draw, with no windows — the one file that has rows, its header row at its own true index with no other index in the map, every anchor naming it and no other, the file drawn exactly as the full render draws it, the render that is unchanged when it is told nothing, the soloed split's two panes of one file's height with one header row between them, the collapsed file that keeps its header, and the empty scope that draws an empty review |
