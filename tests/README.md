...
- **A hunk with a section heading cannot come from any fixture in this suite.** git appends
  ...
  produces seven hunks and not one heading among them. So `split_spec`'s
  `keeps git's section heading with the post-image range` loops looking for one, finds none,
  and has passed since it was written without asserting anything. `chrome_spec` embeds a diff
  git wrote and puts it through `diff.parse` instead of teaching a fixture a function:
  `header` and `heading` then agree the way git's own output makes them agree, which a
  hand-written `CRHunk` pair cannot be trusted to do, and no count or row assertion in nine
  other specs has to move.
