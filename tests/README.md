...
| `capture_spec` | Annotating from an ordinary buffer: scope, types, declining one — with the type picker offering every type behind its own glyph and declining behind the untyped mark, read as each label's first non-blank run because `find` alone cannot fail on an empty glyph — blob, composer, diagnostics, restart, one queue, the immediate send |
