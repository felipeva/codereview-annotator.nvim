...
| `codereview/edit_child.lua` | Spawned by `edit_restart_spec` to queue three notes from a buffer, edit the middle one and exit with nothing written after the edit — deliberately not a spec. |
...
| `queue_float_spec` | How the float draws an entry: the bar down every row it owns, the boundary between two, notes kept and wrapped by display width, dropping and editing from anywhere inside one, where the cursor lands after an edit, an entry from the capture path and a bare note edited alike, the mark it spends on a stale entry, learned off the row and refused to every annotation type, and the two keys that act on the whole batch — one closing the float, one leaving it open |
...
| `edit_restart_spec` | An edited note across a genuine restart: the next session restores the new note, every id as it was, and the edited entry in the place it held |
