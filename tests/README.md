...
- **A bar reading cannot see a group the theme does not define, and reports the bar's own
  ...
  unless the group is defined first: it reads the bar's own colour whether the bar asked for
  the host's group or never asked for anything. `file_icon_spec`'s bar case defines the group,
  takes the reading, puts the definition back, and only then asserts, which is the fade case's
  discipline two acts above it and is here for a different reason: there the definition would
  leak into every case below, here its *absence* would make the case unable to fail. Measured
  rather than reasoned about — `%#Undefined#x%*` through `nvim_eval_statusline` reports
  `groups = { "WinBar" }` and nothing else. A group this plugin owns needs none of this: `hl.lua`
  links every one of them at `setup`, so they are defined in any process that opened a review.
