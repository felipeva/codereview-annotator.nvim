...
| `codereview/frame_child.lua` | Spawned three times by `frame_spec`, one painted cell each — the pane's last column on the blank **pad** row inside a muted pane, and two cells of a file header row, one under a column mark and one not — deliberately not a spec. |
...
| `frame_spec` | The **frame**: the rule above and below a file's body. The four groups first — definitions rather than links, because a link cannot add an attribute to what it copies; underlined in `cterm` as well as in gui; reaching for no `overline`; named where `hl.groups()` reads, with a fade twin each that keeps the underline; and each pair's two edges resolving to one foreground, guarded by a case proving the two pairs are different colours. Then which row carries which line-wide group, at the pure-data seam: an expanded file's two edges on its own header and pad anchors with nothing framed between them, a collapsed file and a reviewed one carrying the top edge alone, a reviewed file expanded again drawing both edges from the reviewed pair, the last file, a review of one file, a review of none, a file with two hunks whose *last* pad row alone closes it, a file whose whole body is a note, both panes of a split framing the same rows in the same groups, and the mark count that moves by one when a file collapses. Then three painted cells in child processes, which is the only claim a group name cannot make: the bottom rule muted with its pane, and the top rule leaving a column mark on the header row its own colour while the rest of that row keeps the header's — read as two absolute colours *and* as an inequality, because each absolute reading passes for one reason and fails for two. |
