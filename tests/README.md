...
| `path_spec` | How a file's path is styled, at the seam that decides it and with no review, no fixture and no repository: the segments the label answers with — a plain path's quiet and bright halves, the cut at the *last* separator, a root-level file with no quiet half at all, a path that is not ASCII, a rename's two paths and its chrome arrow in the unified layout and one path per pane in the split one, the pre-image half a file added on the branch does not have, and a name carrying a `%` staying a literal and drawing as itself when a bar is assembled from it — beside the fields every other surface goes on reading off that label |
